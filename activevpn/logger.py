...
from activevpn.config import LOG_FILE
from activevpn.detector import ScanResult
...
def save_log(scan_data: Union[ScanResult, dict]) -> bool:
    """
    Appends the current scan results to the history JSON file.

    Accepts either a :class:`activevpn.detector.ScanResult` (library usage)
    or a plain dict of scan results.
    """
    if isinstance(scan_data, ScanResult):
        scan_dict = scan_data.to_dict()
    else:
        scan_dict = scan_data
